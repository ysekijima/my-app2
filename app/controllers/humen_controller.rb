class HumenController < ApplicationController
  ...
  def index
    if params[:q].present?
      # @humen = Human.where('name LIKE ?', "%#{params[:q]}%")
      @humen = Human.where("name LIKE '%#{params[:q]}%'").order(number: :asc)
    else
      @humen = Human.all.order(number: :asc)
    end
  ...
  def destroy
    ...
    respond_to do |format|
      format.html { redirect_to humen_path, status: :see_other, notice: "Human was successfully destroyed." }
  ...
  private
    ...
    def human_params
      params.require(:human).permit(:name, :size, :memo, :number)
    end
end
